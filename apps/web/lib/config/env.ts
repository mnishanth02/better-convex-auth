 ...
 */
export function isOAuthProviderConfigured(provider: "google" | "github" | "apple" | "discord"): boolean {
  if (typeof window !== "undefined") {
  ...
  const [idKey, secretKey] = providerMap[provider];
  return Boolean(serverEnv[idKey as keyof Env] && serverEnv[secretKey as keyof Env]);
}
